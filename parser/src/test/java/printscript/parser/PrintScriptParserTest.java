...
class PrintScriptParserTest {
    ...
    void parsesAssignmentStatement() {
        List<Token> tokens = List.of(
                token(TokenType.IDENTIFIER, "x"), token(TokenType.ASSIGN, "="),
                token(TokenType.NUMBER_LITERAL, "5"), token(TokenType.SEMICOLON, ";"),
                token(TokenType.EOF, "")
        );
    ...
    void parsesBinaryExpressionsWithAllOperators() {
        // a + b - c * d / e
        List<Token> tokens = List.of(
                token(TokenType.LET, "let"), token(TokenType.IDENTIFIER, "r"), token(TokenType.COLON, ":"),
                token(TokenType.NUMBER_TYPE, "number"), token(TokenType.ASSIGN, "="),
                token(TokenType.IDENTIFIER, "a"), token(TokenType.PLUS, "+"), token(TokenType.IDENTIFIER, "b"),
                token(TokenType.MINUS, "-"), token(TokenType.IDENTIFIER, "c"), token(TokenType.STAR, "*"),
                token(TokenType.IDENTIFIER, "d"), token(TokenType.SLASH, "/"), token(TokenType.IDENTIFIER, "e"),
                token(TokenType.SEMICOLON, ";"), token(TokenType.EOF, "")
        );
